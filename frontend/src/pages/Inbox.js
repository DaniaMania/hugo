    ...
    setEmails([
      {
        ...
        tags: ["urgent"],
        content:
          "We regret to inform you that the delivery of battery packs (PO #1221) will be delayed by 2 weeks due to supply chain issues. New ETA: April 5th.",
        aiInsights: {
          summary:
            "This email indicates a 2-week delay for PO #1221 (batteries). Recommend updating the S2 production forecast and notifying sales.",
          suggestedActions: [
            "Update PO Status",
      ...
      },
      {
        ...
        tags: ["price-change"],
        content:
          "Due to increased material costs, we need to adjust the price of electric motors by 5%. New price list attached.",
        aiInsights: {
          summary:
            "Price increase notification for electric motors. Impact on production costs needs assessment.",
          suggestedActions: [
            "Review Price Impact",
            "Generate Renegotiation Request",
          ],
        },
      },
    ]);
    ...
    axios.get('http://localhost:4000/emails', {
      ...
      .then(response => {
        ...
        const fetchedEmails = response.data.map(email => ({
          ...
          aiInsights: {
            summary: email.aiInsights.summary,
          },
        }));
    ...
    <div className="flex h-full">
      ...
      {selectedEmail && showAIPanel && (
        <div className="w-96 border-l dark:border-gray-700 p-4">
          ...
          <div>
            {selectedEmail.aiInsights.suggestedActions && (
              <div>
                <h3 className="font-medium mb-2 text-gray-800 dark:text-gray-200">
                  Suggested Actions
                </h3>
                <div className="space-y-2">
                  {selectedEmail.aiInsights.suggestedActions.map((action) => (
                    <button
                      key={action}
                      className="w-full px-4 py-2 text-left text-sm rounded-lg bg-gray-300 dark:bg-gray-700 text-gray-800 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-600"
                    >
                      {action}
                    </button>
                  ))}
